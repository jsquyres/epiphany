...
def convert_pledges_to_ps_import(ps_families, jotform, log):
    ...
    for pledge in jotform:
        ...
        fduid = int(fduid)
        if fduid not in ps_families:
            log.warning(f"WARNING: Family FDUID {pledge['Family DUID']} / {pledge['Family names']} submitted a pledge, but is no longer in PS")
            continue
        ...
        row['Family DUID']   = pledge['Family DUID']
...
def analyze_member_ministry_submissions(ps_members, ps_families, jotform_csv, log):
...
def member_ministry_csv_report(args, google, start, end, time_period, ps_members, ps_families, jotform_csv, log):
    ...
    output      = analyze_member_ministry_submissions(ps_members, ps_families, jotform_csv, log)
    today       = date.today()
    ...
    for ministry_name in sorted(output.keys()):
        ...
        for category in sorted(data.keys()):
            ...
            for member in data[category]:
                ...
                emails = ParishSoft.find_any_email(member)
                if emails:
...
def main():
    ...
    if True:
        ...
        member_ministry_csv_report(args, google, start, end, time_period,
                                   members, families, jotform_range_list, log)
